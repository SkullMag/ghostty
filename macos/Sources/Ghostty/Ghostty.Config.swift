...
extension Ghostty {
    ...
    class Config: ObservableObject {
        ...
        var macosTabPosition: MacOSTabPosition {
            let defaultValue = MacOSTabPosition.top
            guard let config = self.config else { return defaultValue }
            var v: UnsafePointer<Int8>?
            let key = "macos-tab-position"
            guard ghostty_config_get(config, &v, key, UInt(key.lengthOfBytes(using: .utf8))) else { return defaultValue }
            guard let ptr = v else { return defaultValue }
        ...
        var macosTabSidebarQueue: Bool {
            guard let config = self.config else { return false }
            var v = false
            let key = "macos-tab-sidebar-queue"
            _ = ghostty_config_get(config, &v, key, UInt(key.lengthOfBytes(using: .utf8)))
            return v
        }
...
extension Ghostty.Config {
    ...
    enum MacOSTabPosition: String {
        static let `default` = MacOSTabPosition.top
        case top, left
    }
}
